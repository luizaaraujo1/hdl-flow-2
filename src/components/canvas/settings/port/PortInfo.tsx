...
const PortInfo = ({port, portCategory}: PortInfoProps) => {
  ...
  return (
    <div className="flex items-center">
      ...
      <h3
        className={`text-sm font-semibold ${isUnnamed ? 'text-red-500' : 'text-gray-500'} ml-1`}>
        {!isUnnamed ? port.name : 'Unnamed Port!'}
      </h3>
      <h3 className="ml-2 text-sm font-semibold">| Type:</h3>
      <h3 className="ml-1 text-sm font-semibold">{port.type}</h3>
    </div>
  );
};
